...
export async function GET(request: NextRequest) {
  ...
  let bookingsQuery = supabase
    .from('bookings')
    .select('id, customer_name, customer_email, start_time, duration, total_amount, service_value_cents, reward_grant_id, deposit_amount, remainder_amount, actual_deposit_paid, status, engineer_name, room, band_id, booking_group_id, sweet_spot_addon, engineer_split_pct, created_at')
  ...
  let cancelledQuery = supabase
    .from('bookings')
    .select('id, customer_name, start_time, total_amount, deposit_amount, actual_deposit_paid, deposit_kept, status, created_at, engineer_name')
    .eq('status', 'cancelled')
    .order('created_at', { ascending: false });
