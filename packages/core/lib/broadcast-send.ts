...
export async function sendPendingBroadcast(broadcastId: string): Promise<SendResult> {
  ...
  // Warm the brand cache BEFORE composing (wrapEmail reads footerLine() sync)
  // and use the brand-driven FROM identity for every batch.
  ...
  for (let i = 0; i < recipients.length; i += BATCH_SIZE) {
    ...
    const payload = chunk.map((r) => ({ from, to: r.email, subject, html: fullHtml }));
