...
/**
 * POST /api/admin/engineers/upload-url — signed upload URL for engineer photos.
 * Mirrors /api/admin/content/upload-url: the browser uploads the file directly to
 * Supabase Storage (the `media` bucket, `engineers/` folder — same place event
 * covers + CMS images live), and we return the public URL to store in
 * engineers.photo_url. Body: { fileName }. Returns { signedUrl, publicUrl }.
 */
export async function POST(request: NextRequest) {
  ...
  if (signErr || !signed) {
    console.error('[admin:engineers:upload] signed URL error:', signErr);
